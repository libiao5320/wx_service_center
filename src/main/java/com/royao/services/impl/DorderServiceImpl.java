...
public  class DorderServiceImpl extends BaseServiceImpl<Dorder> implements DorderService {
	...
	public boolean comment(Devaluate devaluate, List l) throws Exception {
		...
		if ( count >  0 ) {
			...
			dorder.setFinnshedTime(DateUtil.current());
			dorder.setSettlementState(-1);
			updateSettlementState(dorder);
			flag  = true;
		}
	...
	public boolean updateSettlementState(Dorder dorder) throws Exception {
		return dorderMapper.finishOrder(dorder) > 0;
	}
	...
	public Integer countUnconsumeOrder(Dorder order) {
		return this.dorderMapper.countUnconsumeOrder(order);
	}

	public boolean clearOrder(Map params)  throws Exception{
		return this.dorderMapper.clearOrder(params) > 0;
	}
